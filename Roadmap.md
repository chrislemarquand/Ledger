...
- [ ] **P5** `Should` **Recents section does not animate on collapse/expand** — root cause identified: custom `if !collapsed { ForEach }` inside `Section` has no lower layout anchor for the bottommost section, so SwiftUI cannot animate it. Fix: replace with `DisclosureGroup(isExpanded: binding)` which animates relative to its own header regardless of list position (same approach as Inspector sections). Not a SwiftUI bug — fixable implementation gap.

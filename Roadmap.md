...
### SwiftUI rendering
- [x] **B14** ✅ **`@Published` mutated during view update** — root cause identified and fixed: `.onChange(of: model.selectedSidebarID)` called `handleSidebarSelectionChange` synchronously, which called `loadFiles` → `clearLoadedContentState` → `browserItems = []` → `rebuildFilteredBrowserItems()` → `filteredBrowserItems = …`, mutating `@Published` properties within the SwiftUI update transaction. Fixed by deferring the call in `Task { @MainActor in … }` so mutations occur outside the view update cycle.
- [x] **B15** ✅ **NSHostingView reentrant layout** — direct downstream consequence of B14; resolved by the same fix. CAMetalLayer zero-size warnings were a secondary symptom of the skipped layout passes.

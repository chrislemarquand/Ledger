...
REQUIRED_VERSION="${EXIFTOOL_REQUIRED_VERSION:-13.51}"
...
if [ -n "${EXIFTOOL_SOURCE_PATH:-}" ] && [ -x "${EXIFTOOL_SOURCE_PATH}" ]; then
  ...
  echo "error: exiftool not found. Place latest exiftool in Vendor/exiftool or set EXIFTOOL_SOURCE_PATH."
  exit 1
fi
...
if [ "${SRC_VERSION}" != "${REQUIRED_VERSION}" ]; then
  ...
  echo "error: update Vendor/exiftool (or EXIFTOOL_SOURCE_PATH) to exiftool ${REQUIRED_VERSION}."
  exit 1
fi
